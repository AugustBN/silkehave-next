      ...
      <section style={{ padding: "96px 0", background: "var(--ceramic)" }}>
        <div style={{ maxWidth: "var(--container-max)", margin: "0 auto", padding: "0 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12, marginTop: 48 }} className="stba-strip">
            {strip.map((p, i) => (
              <ScrollReveal key={p.src} delay={i * 0.06}>
                <figure style={{ margin: 0 }}>
                  <div className="stba-photo" style={{ overflow: "hidden", borderRadius: "var(--radius-md)", marginBottom: 8 }}>
                    <Image
                      src={p.src}
                      alt={p.caption}
                      width={600}
        ...
        <style>{`
          .stba-photo img { transition: transform 600ms cubic-bezier(0.22,1,0.36,1); }
          .stba-photo:hover img { transform: scale(1.04); }
          @media (max-width: 880px) { .stba-strip { grid-template-columns: repeat(3, 1fr) !important; } }
          @media (max-width: 560px) { .stba-strip { grid-template-columns: repeat(2, 1fr) !important; } }
        `}</style>
      </section>
